...
# 这里使用字典来模拟数据库存储，包含一个默认管理员账户，默认密码: admin
fake_users_db = {
    "root":{
        "username": "root",
...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    ...
    Args:
        ...
        hashed_password(str) : 数据库中存储的密码haxi
    Returns:
        bool: 密码是否匹配
    """
...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None) -> str:
    """
    创建JWT访问令牌
    :param data: 要编码到令牌中的数据(通常包含用户标识)
    :param expires_delta: 令牌过期时间，如果不提供则使用默认值
    :return: 编码后的JWT令牌
    """
...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)]) -> UserInDB:
    ...
    Depends:当还没进入方法之前先进入Depends中的方法
